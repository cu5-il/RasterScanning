...
void postPrint(double speed, double augerSpeed) {
	...
	// raise the z axis
	nextPos[0] = 10;
	if (!A3200MotionLinearVelocity(handle, TASK_MOVE, (AXISMASK)(AXISMASK_02), nextPos, 10)) { A3200Error(); }
	...
	if (!A3200MotionMoveAbs(handle, TASK_MOVE, (AXISINDEX)(AXISINDEX_03), 90, 20)) { A3200Error(); }
	//if (!A3200MotionWaitForMotionDone(handle, AXES_ALL, WAITOPTION_MoveDone, -1, NULL)) { A3200Error(); } 
	// move to disposal cup
	nextPos[0] = DISPOSAL_X;
	nextPos[1] = DISPOSAL_Y;
	if (!A3200MotionLinearVelocity(handle, TASK_MOVE, (AXISMASK)(AXISMASK_00 | AXISMASK_01), nextPos, 10)) { A3200Error(); }
...
void printPath(std::deque<std::vector<double>>& path, cv::Point2d initPos, double speed, double augerSpeed) {
	...
	if (!A3200ProgramStop(handle, TASK_MOVE)) { A3200Error(); }

	// Put task into Queue mode.
	if (!A3200ProgramInitializeQueue(handle, TASK_MOVE)) { A3200Error(); }
	...
	if (!A3200MotionMoveAbs(handle, TASK_MOVE, (AXISINDEX)(AXISINDEX_00), initPos.x - 5, 10)) { A3200Error(); }
	if (!A3200MotionMoveAbs(handle, TASK_MOVE, (AXISINDEX)(AXISINDEX_01), initPos.y, 10)) { A3200Error(); }
	if (!A3200MotionMoveAbs(handle, TASK_MOVE, (AXISINDEX)(AXISINDEX_03), 0, 30)) { A3200Error(); }
	if (!A3200MotionMoveAbs(handle, TASK_MOVE, (AXISINDEX)(AXISINDEX_02), -8/*-7.6*/, 3)) { A3200Error(); } //FIXME: manually set starting Z height
	if (!A3200MotionWaitForMotionDone(handle, AXES_ALL, WAITOPTION_MoveDone, -1, NULL)) { A3200Error(); }
	...
	while (!path.empty()){
		...
		while (!A3200MotionLinearVelocity(handle, TASK_MOVE, (AXISMASK)(AXISMASK_00 | AXISMASK_01 | AXISMASK_03), nextPos, speed )) { 
	...
	if( !A3200StatusGetItem(handle, TASK_MOVE, STATUSITEM_QueueLineCount, 0, &queueLineCount)) { A3200Error(); }
	...
	// disable the axes
	if (!A3200MotionDisable(handle, TASK_MOVE, AXES_ALL)) { A3200Error(); } //FIXME: axes are not disabling
